...
func (ic *OutwardController) UpdateOutwardDataController(w http.ResponseWriter, r *http.Request) {
	var material models.MaterialOutward
	err := json.NewDecoder(r.Body).Decode(&material)
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
	...
	err = ic.outwardRepo.UpdateFormOutwardData(material)
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
		utils.Encode(w, map[string]string{"message": "Error updating outward data"})
		return
	}
	w.WriteHeader(http.StatusOK)
	utils.Encode(w, map[string]string{"message": "Outward data updated successfully"})
}
